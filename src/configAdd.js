...
function configAdd(projectUrl, projectName, modelName, menuName) {
  ...
  const menuDataUrl =
    projectUrl + "\\src\\components\\common\\Menus\\menuData.js";
  ...
  if (runFlag < 0) {
    ...
    if (anchorSn >= 0) {
      ...
      if (logoSn >= 0) {
        fs.writeFileSync(
          modelUrl,
          insertFlg(insertContext, insertModel + "\n", logoSn + 18)
        );
        console.log(
          chalk.green("SCMK SUCCESS => 成功写入entry/inventory.js注册model")
        );
      } else
        console.log(
          chalk.red("SCMK ERROR : 注册model写入文件失败！请检查写入权限！")
        );
    } else
      console.log(
        chalk.red(
    ...
    console.log(
      chalk.blue(`SCMK INFO : 检测到模块${modelName}的model已经被注册。`)
    );
  }
  /*
    menu link add
